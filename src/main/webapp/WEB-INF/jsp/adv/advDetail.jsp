...
<body>
		...
		<div class="container pt-100">
				...
				<div class="col-xs-12">
					...
					<div class="about-image float-left hidden-sm">
						<div class="baguetteBoxOne gallery">
							<a href="${advPhotoCover}" data-caption=""> <img src="${advPhotoCover}" alt="">
							</a>
			...
			<div class="row pt-50">
				...
				<form id="searchRoomForm" action="searchRoom" method="post" type="hidden">
					...
					<div class="col-xs-3">
						<a href="javascript:searchRoom()" class="btn btn-primary">Check Available Rooms</a>
					</div>
				</form>
			</div>
		</div>
	...
	<div id="floor-plan-area" class="floor-plan-area section pt-120">
		<div class="container  pt-120" id="roomArea"></div>
	</div>
...
<script src="https://maps.googleapis.com/maps/api/js?key=AIzaSyAQMsV9thPFaPE4jL4Xx5lrCzMWuMKBKcw&libraries=places&callback=initMap" async defer></script>
	...
	function buildIWContent(place) {
      ...
      '">'
		...
		if (place.price_level) {
			...
			document.getElementById('iw-price').textContent = priceHtml;
		} else {
...
<script type="text/javascript">
				...
				.post(
						...
						function(data) {
							...
							for (var i = 0; i < data.total; i++) {
								...
								html += "</div></div><div class='floor-plan-content col-lg-7 col-md-6 col-xs-12'><h2>Room: <span>"
										+ room.name
										+ "</span> $"
										+ room.rental
										+ "</h2><p>"
										+ room.description
										+ "</p><a href='javascript:showTextArea("
										+ room.id
										+ ")' id='bookBtm0"
										+ room.id
										+ "'>Book Now</a><div class='row' id='HabitArea"+room.id+"' style='display: none;'><div class='col-xs-12'><label for='name'>Habit</label><textarea class='form-control' rows='3' id='habit"+room.id+"' name='habit'></textarea></div></div><div class='row' id='ExpectationArea"+room.id+"' style='display: none;'><div class='col-xs-12  '><label for='name'>Expection</label><textarea class='form-control' rows='3' id='expectation"+room.id+"' name='expectation'></textarea></div></div><div class='row pt-30'><div class='col-xs-4 col-md-offset-4 '><a href='javascript:bookRoom("
										+ room.id
										+ ")' class='btn btn-primary' id='bookBtm1"
										+ room.id
	...
	function bookRoom(roomId) {
		...
		var expection = $("#expectation" + roomId).val();
		var myDate = new Date();
		var date = myDate.getDate();
		if (!compareDate(stayFrom, stayTo)) {
			$.messager
					.alert("Warning",
							"Your check-in date should be earlier than the check-out date.");
			return;
		}
		var currentDate = myDate.getDate() + "-" + myDate.getMonth() + "-"
				+ myDate.getFullYear();
		if (!compareDate(currentDate, stayFrom)) {
			$.messager
					.alert("Warning",
							"Your check-in date should be later than the current date.");
			return;
		}
		$.messager
				.confirm(
